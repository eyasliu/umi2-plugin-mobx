...
function getStoresWithRoutes(routes, api) {
  ...
  return routes.reduce((memo, route) => [...memo, ...(route.component && route.component.indexOf('() =>') !== 0 && route.component.indexOf('/layouts/') === -1 ? getPageStores((0, _path.join)(paths.cwd, route.component), api) : []), ...(route.routes ? getStoresWithRoutes(route.routes, api) : [])], []);
}
...
function _default(api, opts = {}) {
  ...
  function getGlobalStoresFiles() {
    return exclude(getGlobalStores(api, shouldImportDynamic), (0, _utils.optsToArray)(opts.exclude)).map(path => {
      ...
      if ((0, _path.join)(path).indexOf((0, _path.join)(paths.absSrcPath, 'pages')) === 0) {
        p.name = (0, _path.basename)((0, _path.join)(path, '../'));
      }
